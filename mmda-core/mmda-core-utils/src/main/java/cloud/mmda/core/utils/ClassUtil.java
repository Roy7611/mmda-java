...
public class ClassUtil {
    ...
    public static @Nullable Method getAnyMethod(@NonNull Class<?> clazz, @NonNull String... methodNames) {
        return getFirstMethod(clazz, method -> CollectionUtil.contains(methodNames, method.getName()));
    }
    ...
    public static boolean isObjectMethod(@NonNull String methodName) {
        return CollectionUtil.contains(OBJECT_METHODS, methodName);
    }
     ...
     * 获取属性值
     *
     * 通过对象和属性的名字获取对象属性的值，如果在当前对象属性没有找到，依次向上收集所有父类的属性，直到找到属性值，没有找到返回null
     * @param obj 对象
     ...
     * @throws IllegalAccessException
     */
    public static void setFieldValue(Object obj, String fieldName, Object val) throws IllegalAccessException,IllegalAccessException {
        Class<?> Clazz = obj.getClass();
        Field field;
        if ((field = getField(Clazz, fieldName)) == null)
            return;
